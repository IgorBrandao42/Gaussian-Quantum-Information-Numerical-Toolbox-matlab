classdef gaussian_state < handle         % class definning a nanoparticle
  ...
  methods
    function obj = gaussian_state(varargin)
      ...
      % Following lines verify if the CM satisfy the uncertainty relation
...
%       assert(all(eig(V_check)>=0), "CM does not satisfy uncertainty relation!")
    ...
    function rho_A = partial_trace_math_notation(rho, indexes)
    ...
    function rho_A = partial_trace(obj, indexes)
      % Partial trace over the complete gaussian state
    ...
    function LN = logarithmic_negativity2(obj, i, j)
      ...
      if obj.N_modes == 2                        % If the full system is only comprised of two modes
        V0 = obj.V;                              % Take its full covariance matrix
      elseif nargin == 3
        bipartition = obj.partial_trace(i, j);   % Otherwise, get only the two mode specified by the user
...
% Alterar classe simulation.m para receber apenas A, N, D das eqs. Langevin e Lyapunov
% Resolver langevin quantica e semi-clássica

% Apenas não testei a log_neg (não sei de cabeça uma CM de estado emaranhado!)
